 ...
 * by the parent page.tsx.
 *
 * The password-reset email link lands here with auth tokens in the URL hash
 * fragment (not sent to the server). We create a browser Supabase client,
 * which automatically exchanges the hash-fragment tokens for cookies via the
 * @supabase/ssr middleware. Once the exchange is complete (or fails), we
 * render the form so the Server Action can read the session from cookies.
 *
 * Also handles ?error=… query params from a failed Supabase redirect.
 */
export function UpdatePasswordPageInner() {
  ...
  useEffect(() => {
    // If we already have an error from query params, skip the hash exchange.
    if (queryError) {
      setReady(true);
      return;
    }
